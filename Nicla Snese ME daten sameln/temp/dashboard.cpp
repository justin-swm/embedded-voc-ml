...
#include "Nicla_System.h"        // Enthält Funktionen und Definitionen für das System, einschließlich der LED-Steuerung und dem Beginn der Kommunikation.
#include "Arduino_BHY2.h"       // Stellt die Schnittstelle zur BHY2-Sensorplatine bereit.
#include <ArduinoBLE.h>         // Bibliothek für Bluetooth Low Energy (BLE) auf Arduino-Platten.

#define BLE_SENSE_UUID(val) ("19b10000-" val "-537e-4f6c-d104768a1214")  // Makro zur Generierung von UUIDs für die BLE-Dienste und -Charakteristiken.

const int VERSION = 0x00000000;   // Versionsnummer des Geräts, definiert als Konstante.

BLEService service(BLE_SENSE_UUID("0000"));  // Definiert einen BLE-Dienst mit einem UUID.

// Definition von Characteristiken für verschiedene Messwerte: Temperatur, Luftfeuchtigkeit, Druck, Gas, Gyroskop, Beschleunigungssensor, Orientierungsquaternion und RGB-LED.
...
BLECharacteristic accelerometerCharacteristic(BLE_SENSE_UUID("5001"), BLERead | BLENotify, 3 * sizeof(float));  // Array von 3 float-Werten für die Beschleunigung.
